...
**Don't write your own auth check.** `middleware/requireAuth.js` is owned by the Account Management epic and is on `develop`: it validates the JWT, attaches `req.user`, and re-reads `accountStatus`/`suspendedAt` live on every request. Apply it in your routes file to every endpoint that needs a signed-in user — a controller that reads `req.user` behind a route without it will always answer 401. See the cross-cutting authentication section in `docs/module-ownership.md` for exactly what it does and doesn't check.
