...
2. Run `date`, `git config user.email` and `git branch --show-current`. Find me in docs/workflow/team.json and check my branch is mine and follows the convention. If it is not, stop and tell me the exact branch to switch to.
3. Read my .worklog/progress.md NEXT block and my latest .worklog/state/ report, if they exist.
4. Run `node scripts/state-report.mjs --save`. If you cannot run commands, tell me and I will paste its output. Do not write code before you have it.
