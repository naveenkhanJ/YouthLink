...
// -------------------------------------------------------- 1. who & where ---
let fetchNote = "fetch skipped: --no-fetch";
if (FETCH) {
  try {
...
say();
const userName = gitOr(["config", "user.name"]);
say(`- **Git author:** ${userName || "**user.name unset** — set it (CONTRIBUTING.md → Commit attribution)"} <${email || "unset"}> → ${me ? `**${me.id}** (${me.role})` : "**not in docs/workflow/team.json** — commits will be blocked and may not be attributed on GitHub"}`);
  ...
  if (!mine.length) say("_None found._");
    ...
    if (unmerged.length) {
      ...
      say(`**Unmerged work in your modules exists on: ${unmerged.map((r) => `\`${r}\``).join(", ")}.** ` +
          ...
          "bringing it onto develop comes before new work (docs/workflow/agent-protocol.md §4.1).");
    }
  }
  say();
}
  ...
  for (const mod of me.modules) {
    ...
    for (const [id, r] of requirements) {
      ...
      scope.push({
        ...
        inWork: (work.ids.get(id) || []).length, inDev: (dev.ids.get(id) || []).length, inUnmerged: unmergedIds.has(id),
...
if (!scope.length) {
  ...
  say();
  say("| Requirement | Priority | Title | Amended | Mentioned (working / develop / unmerged branch) | feat/fix tag (develop / branch) | Screens |");
  say("| --- | --- | --- | --- | --- | --- | --- |");
  for (const s of scope) {
    say(`| ${s.id} | ${s.priority} | ${s.title} | ${s.amended.join(", ") || "—"} | ${s.inWork ? "yes" : "no"} / ${s.inDev ? "yes" : "no"} / ${s.inUnmerged ? "yes" : "no"} | ${s.tagDev ? "yes" : "no"} / ${s.tagBranch ? "yes" : "no"} | ${s.screens || "—"} |`);
  }
}
say();
